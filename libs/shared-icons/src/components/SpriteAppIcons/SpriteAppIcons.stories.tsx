...
import SpriteAppIcons, { appIcons } from './SpriteAppIcons'
...
const meta = {
   component: SpriteAppIcons,
   title: 'Components/SpriteAppIcons',
   parameters: {
      layout: 'centered'
   },
   argTypes: {
...
export const WithSearch: Story = {
   render: args => {
      ...
      const filteredIcons = appIcons.filter(icon =>
            ...
            <div className={args.className || 'grid grid-cols-3 gap-4'}>
               {filteredIcons.length > 0 ? (
                  filteredIcons.map(icon => (
                     ...
                     >
                        {SpriteAppIcons.renderIcon(icon)}
                     </div>
                  ))
               ) : (

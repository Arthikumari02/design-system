...
import SpriteFileIcons, { fileIcons } from './SpriteFileIcons'
...
const meta = {
   component: SpriteFileIcons,
   title: 'Components/SpriteFileIcons',
   parameters: {
      layout: 'centered'
   },
   argTypes: {
...
export const WithSearch: Story = {
   render: args => {
      ...
      const filteredIcons = fileIcons.filter(icon =>
            ...
            <div className={args.className || 'grid grid-cols-3 gap-4'}>
               {filteredIcons.length > 0 ? (
                  filteredIcons.map(icon => (
                     ...
                     >
                        {SpriteFileIcons.renderIcon(icon)}
                     </div>
                  ))
               ) : (

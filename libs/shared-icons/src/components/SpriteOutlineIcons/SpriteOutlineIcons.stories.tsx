...
import SpriteOutlineIcons, { outlineIcons } from './SpriteOutlineIcons'
...
const meta = {
   component: SpriteOutlineIcons,
   title: 'Components/SpriteOutlineIcons',
   parameters: {
      layout: 'centered'
   },
   argTypes: {
...
export const WithSearch: Story = {
   render: args => {
      ...
      const filteredIcons = outlineIcons.filter(icon =>
            ...
            <div className={args.className || 'grid grid-cols-3 gap-4'}>
               {filteredIcons.length > 0 ? (
                  filteredIcons.map(icon => (
                     ...
                     >
                        {SpriteOutlineIcons.renderIcon(icon)}
                     </div>
                  ))
               ) : (
